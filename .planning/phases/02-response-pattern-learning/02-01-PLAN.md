...
<task type="auto">
  <name>Task 1: Create ResponsePattern dataclass and learning engine</name>
...
2. **ResponsePatternLearner class** with `__init__`, `learn`, `_extract_pattern`, and `_parse_json`:
   ...
   - `_extract_pattern(self, endpoint_key, success_response, failure_response)` -- compare the two Evidence objects and build a ResponsePattern:
     - Parse both bodies as JSON (gracefully handle non-JSON via `_parse_json`)
     - Check STATUS_FIELD_CANDIDATES: if a candidate field exists in BOTH responses with different values, record it as `status_field` with the respective success/failure values
     - If a candidate field exists ONLY in failure response, record it as `error_field`
     - Record top-level keys difference as structural fingerprint (`success_keys`, `failure_keys`)
   - `_parse_json(self, body: str) -> dict | None` -- safely parse JSON, return None for non-JSON

Log at INFO level: "Learned response patterns for N/M endpoints". Log at DEBUG level: per-endpoint pattern details (for --verbose mode). Use `logging.getLogger(__name__)`.

IMPORTANT: Do NOT implement `is_real_success()` in this task -- that is Task 2. This task focuses only on learning and pattern extraction.
...
- Only import from stdlib or existing codebase (json, logging, dataclasses, api_pentest.core.models, api_pentest.core.http_client)
  </action>
  <verify>
    python -c "from api_pentest.core.response_patterns import ResponsePatternLearner, ResponsePattern; rp = ResponsePattern(endpoint_key='GET:/test', status_field='status', success_values={'success'}, failure_values={'fail'}, error_field=None, success_keys=None, failure_keys=None); print(f'ResponsePattern: {rp}'); assert hasattr(ResponsePatternLearner, 'learn'); assert hasattr(ResponsePatternLearner, '_extract_pattern'); assert hasattr(ResponsePatternLearner, '_parse_json'); assert not hasattr(ResponsePatternLearner, 'is_real_success'); print('Import and structure OK')"
  </verify>
  <done>
    ResponsePattern dataclass exists with all 7 fields (endpoint_key, status_field, success_values, failure_values, error_field, success_keys, failure_keys).
    ResponsePatternLearner class exists with learn(), _extract_pattern(), _parse_json() methods.
    is_real_success() is NOT on this class yet (deferred to Task 2).
    Module imports cleanly with no errors.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add is_real_success() and wire learner into runner and BaseScenario</name>
  <files>api_pentest/core/response_patterns.py, api_pentest/runner.py, api_pentest/scenarios/base_scenario.py</files>
  <action>
**In `api_pentest/core/response_patterns.py` -- add `is_real_success` method to ResponsePatternLearner:**

Add method `is_real_success(self, evidence: Evidence, endpoint_key: str) -> bool` that checks signals in priority order:
1. HTTP status must be 200-299 (if not, return False immediately)
2. Look up learned pattern for endpoint_key in self.patterns
3. If no pattern learned, return True (fall back to HTTP-status-only -- no regression)
4. Parse response body as JSON via _parse_json()
5. If JSON and status_field exists in pattern: check if value is in failure_values -> return False, in success_values -> return True
6. If JSON and error_field exists in pattern: check if error_field is present in response body -> return False
...
IMPORTANT: Keep `is_success_status()` method unchanged -- it is still used by scenarios for baseline/precondition checks. Do NOT change any scenario files in this task -- that is Plan 02's job.
  </action>
  <verify>
    python -c "
from api_pentest.core.response_patterns import ResponsePatternLearner, ResponsePattern
assert hasattr(ResponsePatternLearner, 'is_real_success'), 'is_real_success not on learner'
print('ResponsePatternLearner.is_real_success OK')

from api_pentest.scenarios.base_scenario import BaseScenario
assert hasattr(BaseScenario, 'is_real_success'), 'is_real_success not on BaseScenario'
...
"
  ...
  <done>
    ResponsePatternLearner.is_real_success() method exists with hierarchical 8-step checking.
...
2. `python -c "from api_pentest.core.response_patterns import ResponsePatternLearner; assert hasattr(ResponsePatternLearner, 'is_real_success')"` -- method exists on learner
3. `python -c "from api_pentest.scenarios.base_scenario import BaseScenario; b = BaseScenario.__new__(BaseScenario); assert hasattr(b, 'is_real_success')"` -- method exists on base
4. `python -c "from api_pentest.runner import PentestRunner; import inspect; assert 'response_learner' in inspect.getsource(PentestRunner.run)"` -- runner integration present
5. All existing scenario imports still work (no broken imports): `python -c "import api_pentest.scenarios.s06_privileged_access; import api_pentest.scenarios.s09_business_flow; import api_pentest.scenarios.s13_unsafe_consumption; print('All imports OK')"`
</verification>

<success_criteria>
- ResponsePatternLearner exists with learn(), _extract_pattern(), _parse_json(), and is_real_success() methods
- ResponsePattern dataclass exists with all 7 fields
