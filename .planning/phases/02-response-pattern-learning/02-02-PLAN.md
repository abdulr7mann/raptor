...
<task type="auto">
  <name>Task 2: End-to-end verification against VAmPI</name>
  <files></files>
  <action>
Run the full pentest scan against VAmPI targeting only S06, S09, and S13 scenarios, and verify that the 10 false positives from HTTP 200 + fail body are eliminated.

1. Check if VAmPI is accessible:
   ```bash
   # Read the config to find VAmPI URL
   cat vampi_config.yaml
   ```

2. Run the scan with only S06, S09, S13:
   ```bash
...
3. Examine the output:
   ...
   - Check that none of the findings are from HTTP 200 + `{"status": "fail"}` responses
   - If VAmPI is not running (connection refused), verify by code inspection instead:
     - Confirm is_real_success() is called at all attack validation points
     - Confirm ResponsePatternLearner handles the `{"status": "fail"}` pattern
     - Log the verification result noting VAmPI was unavailable for live test

4. Also run a quick full scan to verify no regressions in other scenarios:
   ```bash
   python run_pentest.py --config vampi_config.yaml --input vampi_openapi.json
   ```

If VAmPI is not available, the verification is still valid through code-path analysis -- the implementation is deterministic and the pattern is well-defined. Note in the summary that live verification is pending VAmPI availability.
  </action>
  <verify>
    - If VAmPI available: Scan output shows 0 findings from HTTP 200 + fail body false positives in S06/S09/S13
    - If VAmPI unavailable: Code inspection confirms all attack validation paths use is_real_success() and learner detects {"status": "fail"} pattern
    - No Python import errors or runtime crashes
  </verify>
  <done>
    False positives from HTTP 200 + fail body are eliminated (confirmed via live scan or code-path analysis).
    No regressions in other scenarios.
    Phase 2 success criteria met: toolkit checks both HTTP status AND response body structure.
  </done>
</task>
...
1. "Running a scan against VAmPI produces zero false positives from HTTP 200 + fail body pattern" -- Verified by Task 2 scan or code-path analysis
...
- VAmPI scan (or code analysis) confirms 0 false positives from HTTP 200 + fail body
