...
3. **Add _discover_from_url() method** (Colorama imports Fore, Style already exist in runner.py header, reuse for progress feedback):
   ```python
...
4. **Add _parse_discovered_spec() helper method** (If InputDetector.parse() raises exception, log error and return empty list to trigger Stage 2 endpoint fuzzing fallback):
   ```python
